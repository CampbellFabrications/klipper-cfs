...
class GCodeMove:
    ...
    def cmd_CX_RESTORE_GCODE_STATE(self, print_info, file_name_path, XYZET):
        ...
        try:
            ...
            #else:
            #    
            logging.info("power_loss cmd_CX_RESTORE_GCODE_STATE G1 X%s Y%s F3000" % (state['last_position'][0], state['last_position'][1])) # Un-indented 
            gcode.run_script_from_command("G1 X%s Y%s F3000" % (state['last_position'][0], state['last_position'][1]))                      # Un-indented
